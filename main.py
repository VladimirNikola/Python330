...
# print(s.find("l"))  # Производит поиск слева на право(индекс)
# print(s.rfind("l"))  # Производит поиск с право на лево(индекс)
# print(s.index("l"))  # Производит поиск слева на право(индекс)
...
# print("       py         ".strip())  # убирает пробельные символы с право и слева
...
# # print(re.search(reg, s).start())  # Начало совпадения
...
# Создать класс Rectangle, описывающий прямоугольник. В классе должны быть все необходимые методы, а так же методы вычисления площади, периметра и диагонали, и метод, который рисует прямоугольник.
...
# }
# print(json.dumps(x))
# print(json.dumps(x, ensure_ascii=False))  # ensure_ascii=False - Кодировка "Виктор"
...
# def get_salary(s):
#     pattern = r"\d+"
#     # res = re.findall(pattern, s)[0]  # 1 вариант
#     res = re.search(pattern, s).group()  # 2 вариант
#     print(res)
#
#
# f = open('index.html', encoding="utf-8").read()
...
#
# def write_csv(data):
#     with open("plugin1.csv", "a", encoding="utf-8-sig") as f:  # -sig - поддержка чтобы картинки переформатировались
#         writer = csv.writer(f, lineterminator='\r', delimiter=";")
...
#
# with sqlite3.connect("cars.db") as con:
#     con.row_factory = sqlite3.Row  # дает возможность обращаться к ключу
#     cur = con.cursor()
#     cur.executescript("""
#     CREATE TABLE IF NOT EXISTS car(
#          car_id INTEGER PRIMARY KEY AUTOINCREMENT,
...
# row = cur.fetchone()  # - возвращает 1 элемент из списка
# print(row)
# row2 = cur.fetchmany(5)  # - возвращает заданное число из списка
# print(row2)
# row3 = cur.fetchall()  # - возвращает весь список
# print(row3)
...
#     print("*" * 60)
#
#     for it, gr in session.query(Lesson.lesson_title, Group.group_name).filter(and_(association_table.c.lesson_id == Lesson.id, association_table.c.group_id == Group.id, Group.group_name == 'MDA-9')):
#         print(it, gr)
#     print("*" * 60)
#
...
# tpl = "{{ cs | random }}"
# tpl = "{{ cs | replace('model', 'brand') }}"  # replace - меняет название ключа
